...
<body>
    <div style="line-height:1.5; font-family:'Golos Text', sans-serif; font-family:'Golos Text', sans-serif; font-size:1rem; text-align:center">
        <main style="padding-block:2rem; padding-inline:2rem">
            <h1 style="color:#6F88FC; font-size:2rem; font-weight:bold">TASK SUBMISSION</h1>
            <p style="color:#191A44">Dear StartEase User, a submission to the task <b>{{ $task->title }}</b> of the project
                <b>{{ $task->project->trademark_name }}</b> by the member {{ $member->person->first_name }} {{ $member->person->last_name }}.
